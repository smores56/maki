...
The `base` field specifies which built-in provider to inherit the model catalog from. Valid values: `anthropic`, `copilot`, `deepseek`, `google`, `llama-cpp`, `mistral`, `ollama`, `openai`, `opencode`, `opencode-go`, `openrouter`, `synthetic`, `tensorx`, `zai`.
