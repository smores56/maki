...
fn snapshot_rows(snap: &KeymapSnapshot, ctx: KeybindContext) -> Vec<(Vec<String>, String)> {
    snap.entries
        .iter()
...
fn hardcoded_rows(ctx: KeybindContext) -> Vec<(Vec<String>, String)> {
    KEYBINDS
        .iter()
...
fn merged_rows(snapshot: &KeymapSnapshot, ctx: KeybindContext) -> Vec<(Vec<String>, String)> {
...
impl HelpModal {
    ...
    pub fn view(&mut self, frame: &mut Frame, area: Rect, snapshot: &KeymapSnapshot) -> Rect {
        ...
        let col_width = all_contexts()
            .flat_map(|c| {
                let snap = snapshot_rows(snapshot, c);
        ...
        for ctx in all_contexts() {
            ...
            let top = merged_rows(snapshot, ctx);
            if top.is_empty() {
                continue;
            }
            if !first {
            ...
            for child in all_contexts() {
                ...
                let child_rows = merged_rows(snapshot, child);
