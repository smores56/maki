...
/// Owner prefix for specs a bundled Lua provider registers via
/// `maki.api.register_provider`. The provider ships with maki, so its
/// [`Source`] is [`Source::Builtin`] (kept out of `list_models`' non-builtin
/// fan-out filter and allowed to claim a slug the inventory reserves), but the
/// owner stays non-`None` so `provider_scope` rollback and `/reload` teardown
/// can target it via [`clear_owner`].
pub const LUA_OWNER_PREFIX: &str = "lua:";
...
pub struct ProviderSpec {
    ...
    /// Declared effort dialect, owned so a Lua `register_provider` call can
    /// build it from a runtime table. When set, the codec applies
    /// `reasoning_effort` from the spec's [`ThinkingConfig`] itself (§5.2), so
    /// a Lua `on_request` hook never touches the field and the wire body stays
    /// order-stable.
    pub effort: Option<crate::types::EffortSpec>,
    pub hooks: Option<Arc<dyn crate::hooks::ProviderHooks>>,
    pub build: BuildFn,
}
...
impl ProviderSpec {
    pub fn source(&self) -> Source {
        match &self.owner {
            ...
            Some(o) if o.as_ref().starts_with(LUA_OWNER_PREFIX) => Source::Builtin,
            Some(_) => Source::User,
        }
    }
}
...
/// Removes a single slug from the registry. Used by `provider_scope` to roll
/// back a spec registered during a scope that then failed, without touching
/// specs the same owner committed in earlier scopes.
pub fn remove(slug: &str) {
    let mut guard = REGISTRY.write().unwrap();
    guard.remove(slug);
}
